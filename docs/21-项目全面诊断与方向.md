...
- **CI 策略**：GitHub Actions 后端已从 `mvn compile` 升级为 `mvn test`，前端保留 `npm run build`。
...
- CI 已升级为 `mvn test` + `npm run build` + Docker smoke；但覆盖仍集中在少数新模块
- 无 E2E、无负载测试（排队并发未验证）
...
| ci-run-tests | 已完成：CI 后端从 `mvn compile` 升级为 `mvn test`，前端保留 `npm run build` | Done |
