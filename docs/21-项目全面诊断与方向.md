...
#### 1. Git 仓库处于「扁平化迁移中途」，状态危险

- **现象**：磁盘上项目已在根目录（[`backend/`](../backend/)、[`frontend/`](../frontend/)、[`docs/`](.)），但 Git 远程仍跟踪旧路径 `MyFirstAiProject/MyFirstAiProject/`；约 800 个 staged deletion + 大量 untracked 根目录文件。
- **风险**：若只 push 当前 staged 删除而不 add 根目录文件，**远程仓库会被清空**。
- **附带问题**：
  - 历史提交含 **483 个 JDK 二进制**（体积大、与 CI 的 Temurin 21 不一致）
  - [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) 在根目录但未提交；远程 CI 路径错误，**Gitee 上 CI 很可能从未真正跑过**
  - 根级 [`.gitignore`](../.gitignore) 仍忽略 `application-prod.yml`；但 [`docker-compose.yml`](../docker-compose.yml) 使用 `SPRING_PROFILES_ACTIVE=prod`，若扁平化提交遗漏生产 profile，远端 clone 后 Docker 后端可能无法启动
  - [`db/salon_data_only.sql`](../db/salon_data_only.sql) 含 `audit_log` 历史请求参数，出现明文登录密码测试数据；提交前需要脱敏或排除
  - [`docs/15-部署与运维手册.md`](15-部署与运维手册.md) / [`README.md`](../README.md) 描述扁平结构，与 remote 不一致

**建议**：下一步开发前，先做 **一次原子 commit**：删除嵌套目录 + 添加根目录项目 + 根级 CI + 修正 `.gitignore`（排除 JDK、`awesome-design-md-main-1/`、日志等，但保留可提交的 `application-prod.yml` 模板）+ 脱敏测试数据。
...
- CI 仅 `mvn compile` + `npm run build` + Docker smoke，**不跑 `mvn test`**
- 无 E2E、无负载测试（排队并发未验证）
...
4. CI 增强：`mvn test` + 可选轻量 smoke（不必保留完整 Docker job 直到 Gitee Runner 稳定）
...
| ID | 工作项 | 优先级 |
|----|--------|--------|
| git-flatten | 完成仓库扁平化迁移：原子 commit 根目录结构 + 根级 CI + .gitignore 清理 JDK/杂项目录 | P0 |
| prod-config-track | 修正 `.gitignore` 与提交策略：保留可提交的 `application-prod.yml` 模板，确保 Docker `prod` profile 可复现启动 | P0 |
| data-sanitization | 脱敏/清理 `db/salon_data_only.sql` 等导出中的 audit_log 请求参数和测试密码，避免把敏感样例带入远端仓库 | P0 |
| ci-run-tests | CI 从 `mvn compile` 升级为 `mvn test`，保留 `npm run build` 与轻量 smoke | P0 |
