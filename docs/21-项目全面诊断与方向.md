...
- **Docker 启动修复**：已修复 `prod` profile 下 `wechatPayGateway` / `alipayGateway` 两个 Bean 同时 `@Primary` 且字段级 `@Qualifier` 未传递到 Lombok 构造器导致的后端启动失败；本地 `prod + MySQL` 验证 `/actuator/health` 为 `UP`。
...
| 问题 | 位置 | 风险 |
|------|------|------|
| ~~Controller 返回 Entity~~ | [`BirthdayConfigController.java`](../backend/src/main/java/com/salon/marketing/controller/BirthdayConfigController.java) | 已修复：生日营销配置改为 `BirthdayConfigVO` 返回、`BirthdayConfigReqDTO` 入参，契约保持一致 |
| ~~多表写无 `@Transactional`，且扣款后流水 rows 未检查~~ | [`CustomerPortalController.pay()`](../backend/src/main/java/com/salon/customer/controller/CustomerPortalController.java) | 已修复：事务包裹余额支付，流水 rows 检查，余额不足回滚，返回余额改用 JDBC 查询避免 MyBatis 一级缓存风险 |
| 未检查 `jdbcTemplate.update` rows | `NotificationController`、`GroupBuyOrderServiceImpl` 等 ~10 处 | 静默失败 |
| ~~定时任务无分布式锁~~ | [`GroupBuyExpiryScheduler`](../backend/src/main/java/com/salon/groupbuy/scheduler/GroupBuyExpiryScheduler.java) | 已修复：使用 MySQL `GET_LOCK('group_buy_expiry', 0)`；锁释放注册到事务完成回调，避免锁先释放但退款事务未提交 |
| ~~审计日志脱敏不足~~ | [`AuditLogAspect.java`](../backend/src/main/java/com/salon/audit/aspect/AuditLogAspect.java) | 已修复：运行时审计参数改为递归字段级脱敏，覆盖 `password/token/secret/authorization/apiKey/accessKey/privateKey/smsCode` 等字段与同名方法参数 |
...
2026-06-17 已按 `docs/20` 完成一轮浏览器验收与兼容补齐：

- 桌面日视图保留技师列网格，并修正预约卡片时间轴定位
- 移动端日视图降级为按时间排序的列表式时间线，避免压缩技师网格
- 周视图移动端保持 3 列紧凑布局，无横向溢出
- 补齐空态/错误态、未分配预约展示、业务时间配置读取
- P1 增强仍可评估：拖拽改期、冲突可视化、批量筛选与打印
...
| P1 | `GroupBuyExpiryScheduler` 锁 | 已完成：多实例锁 + 事务完成后释放；`CustomerPortalController.pay`、审计日志敏感字段脱敏、BirthdayConfigController VO 已修复 |
...
1. **预约日历 P1 增强评估**（[`docs/20`](20-预约日历看板需求文档.md) P0/P1 兼容补齐已完成）：拖拽改期、冲突可视化、批量筛选与打印
2. **报表/仪表盘 WebSocket 或短轮询优化**（可选 P2）
3. **前端 polish 一轮**（可复用 `frontend-polish` skill）：继续推广空态/载态/移动端 H5 细节
...
Week 3: 轨道 C 预约日历 P1 增强评估 + 高频前台体验优化
...
```
优先: 预约日历 P1 增强评估 → 营销秒杀/拼团增强 → AI 应用层
...
- **最大产品差距**：营销（39%）+ 微信生态；预约日历 P0/P1 兼容补齐已完成，后续是拖拽/冲突可视化等增强体验
- **最大架构债务**：Redis 未接入 — 单实例可跑，上线/扩容必做
- **代码质量**：主路径已商用级；剩余 ~10 处 jdbcTemplate/事务问题集中在 H5 门户、拼团、通知等较新模块

**默认推荐路径**：先 **轨道 A + B（工程健康 + 试点上线集）**，产品侧继续做 **预约日历 P1 增强评估（轨道 C）**；AI 应用层在试点有数据后再做流失预测 MVP。
...
| docker-prod-payment-gateway | 已完成：修复 `prod` profile 支付网关注入冲突，后端健康检查可返回 `UP` | Done |
| redis-phase9 | 上线前接入 Redis：分布式锁、限流、缓存、支付幂等（docs/12 Phase 9） | P0 |
| real-pay-sms | 申请商户资质并接入真实微信/支付宝 SDK + 阿里云 SMS | P0 |
| code-hotfixes | 已完成：CustomerPortalController.pay 事务/流水 rows、审计日志敏感字段脱敏、BirthdayConfigController VO、GroupBuyExpiryScheduler 多实例锁均已修复 | Done |
| audit-log-mask | 已完成：审计日志运行时补充 password/token/secret/authorization/apiKey/accessKey/privateKey/smsCode 等字段级脱敏，避免新日志继续记录敏感参数 | Done |
| appointment-calendar-acceptance | 已完成：浏览器验收 + 移动端时间线 + 空态/错误态 + 未分配预约展示；剩余拖拽改期/冲突可视化/批量筛选与打印属 P1 增强评估 | Done |
